...
func TestBasicRules(t *testing.T) {
	testCases := []struct {
		...
		{"==", New[int]().EqualFunc(New[int](), Equal[int]), true},
		{"==", New(0, 0, 1).EqualFunc(New(1, 0, 0), Equal[int]), false},
		{"==", New(1, 2, 3).EqualFunc(New(1, 2, 3), Equal[int]), true},

		{"!=", !New[int]().EqualFunc(New[int](), Equal[int]), false},
		{"!=", !New(0, 0, 1).EqualFunc(New(1, 0, 0), Equal[int]), true},
		{"!=", !New(1, 2, 3).EqualFunc(New(1, 2, 3), Equal[int]), false},

		{"<", Less("a", "z"), true},
		{"<", Less('z', 'a'), false},
		{"<", Less('o', 'o'), false},

		{"<=", LessOrEqual(0.1, 1.2), true},
		{"<=", LessOrEqual(1e69, 420), false},
		{"<=", LessOrEqual(1, 1), true},

		{">", Greater[byte]('a', 'A'), true},
		{">", Greater[byte]('N', 'n'), false},
		{">", Greater[byte]('z', 'z'), false},

		{">=", GreaterOrEqual(0b110100100, 0b1000101), true},
		{">=", GreaterOrEqual(0x0, 0xF), false},
		{">=", GreaterOrEqual(0, 0), true},
	}
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			if tC.got != tC.want {
...
func TestCompFunc(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			if tC.got != tC.want {
				t.Errorf("Error: %s got %t, want %t.", tC.desc, tC.got, tC.want)
			}
		})
	}
}
